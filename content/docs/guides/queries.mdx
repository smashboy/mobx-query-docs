...
| Class               | Returns       | Suspense hooks                                       | Use case                                             |
| ------------------- | ------------- | ---------------------------------------------------- | ---------------------------------------------------- |
| `QueryMany`         | Entity array  | `useSuspenseQuery` / `useDeferredQuery` / `useQuery` | Fetching a list of entities (e.g. all folders)       |
| `QueryOne`          | Single entity | `useSuspenseQuery` / `useDeferredQuery`              | Fetching a single entity by ID                       |
...
A variant of `useSuspenseQuery` that uses React's `useDeferredValue` internally. This is the **recommended hook for search or filter inputs**.

When `args` change, React will keep showing the previous results while fetching the new ones in the background, preventing the UI from "flashing" a loading state/spinner during active search.

```tsx
const search = useSearch(); // e.g. from TanStack Router
const documents = folder.documentsQuery.useDeferredQuery(search);
```
...
If your query doesn't need arguments, simply call the hook with no arguments.
...
`QueryOne` provides two suspense-enabled hooks:

#### `useSuspenseQuery(args)`

Returns a **single entity instance** and suspends if not loaded:
...
Same as `QueryMany.useDeferredQuery()`. Uses `useDeferredValue` to keep the previous entity on screen while fetching a new one (useful for "Next/Previous" navigation or selection changes).

```tsx
const folder = folderByIdQuery.useDeferredQuery(selectedId);
```
...
| `'recent'`          | Your `queryKey()`       | Your custom logical identifier                                       |
...
1. **Bulk invalidation** — After a `CreateMutation` with `invalidationStrategy: 'all-entity-queries'`, mobx-query can call `queryClient.invalidateQueries({ queryKey: ['Folder'] })` to invalidate _every_ query related to the `Folder` entity.
...
1. A parent query fetches documents **with their labels joined**:
   ...
   const documentsQuery = new QueryMany({
     ...
     queryKey: () => ["all"],
     queryFn: async () => {
       // Fetch documents with labels included in the response
