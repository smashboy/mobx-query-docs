---
title: Advanced
description: Register custom typed context, build reusable query/mutation factories, and more.
---
...
import type { MQClientContext as MQClientContextBase } from "mobx-query/types";
...
import { EntityConstructorAny } from "mobx-query/entity/Entity";
...
import { EntityConstructorAny, EntityAny } from "mobx-query/entity/Entity";
import {
  OptimisticMutationInvalidationStrategy,
  OptimisticMutationErrorStrategy,
} from "mobx-query/mutations/types";
...
export function createUpdateMutation<TEntity extends EntityConstructorAny>(
  ...
  return new UpdateMutation({
    ...
    mutationFn: async () => {
      ...
      await fetch(`/api/${endpoint}/${instance.id}`, {
        method: "PATCH",
        body: JSON.stringify(body),
        headers: { "Content-Type": "application/json" },
      });
...
export function createDeleteMutation<TEntity extends EntityConstructorAny>(
  ...
  return new DeleteMutation({
    ...
    instance,
    mutationFn: async () => {
      await fetch(`/api/${endpoint}/${instance.id}`, { method: "DELETE" });
    },
  });
}
```
...
> extends Entity<TId, TData> {
  ...
  readonly updateMutation = new UpdateMutation({
    ...
    instance: this,
    mutationFn: async () => {
      await fetch(`/api/${this.getEndpoint()}/${this.id}`, {
        method: "PATCH",
        body: JSON.stringify(this.getUpdateBody()),
        headers: { "Content-Type": "application/json" },
      });
    },
  });
...
```ts title="folder.entity.ts"
export class Folder extends CrudEntity<string, FolderData> {
  id: string = crypto.randomUUID();
