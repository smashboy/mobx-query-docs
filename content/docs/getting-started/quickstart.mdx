...
const TodoList = observer(() => {
  ...
  const todos = rootStore.todos.todosQuery.useSuspenseQuery(undefined);
...
<Callout type="success">
  **That's it!** You now have a fully reactive setup where: -
  ...
  action — and the UI re-renders instantly via `observer()`. - If the same
  `Todo` entity appears in other queries, it's the **same object instance**, so
