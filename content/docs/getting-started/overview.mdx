...
const folders = foldersQuery.useSuspenseQuery(undefined);
...
5. **On success:** the entity is confirmed, related queries are invalidated based on the configured strategy (`'all-queries'` or `'related-queries'`).
