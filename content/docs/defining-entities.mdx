...
When you create a new entity via [`CreateMutation`](/mutations/store-mutations#createmutation), mobx-query immediately instantiates the entity and adds it to the `EntityCollection` **before** the server request completes. For this to work, the entity needs a valid, unique `id` at instantiation time.
...
<Callout type="warn">
  ...
  [`CreateMutation`](/mutations/store-mutations#createmutation). Use
...
- You need an ID before [`CreateMutation`](/mutations/store-mutations#createmutation) is called.
...
The state transitions are managed automatically by the [`OptimisticMutationStrategy`](/mutations/optimistic-strategies):
...
[`UpdateMutation`](/mutations/entity-mutations#updatemutation) integrates with dirty tracking out of the box. It automatically **skips mutations** if the entity is not dirty, preventing unnecessary server requests:
...
On a **successful mutation**, the dirty state is automatically cleared via `_clearDirty()`. On a **failed mutation** with the [`'rollback'` error strategy](/mutations/optimistic-strategies#error-strategies) (the default), `reset()` is called automatically, reverting the entity to its original values.
